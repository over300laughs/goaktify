...
func (c *Campaign) Create() error {
	insertQuery := `INSERT INTO campaigns (id, name, description, created_on, updated_on, is_active) VALUES (nextval('campaigns_id_sequence'), $1, $2, NOW(), NOW(), $3) RETURNING id`
...
func (c *Campaign) read(id int) error {
	query := `SELECT id, name, description, created_on, updated_on, is_active
